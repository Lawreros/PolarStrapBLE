...
using System;
using System.IO;                            //added, and line 465
...
namespace SDKTemplate
{
    ...
    // This scenario connects to the device selected in the "Discover GATT Servers" scenario and communicates with it.
    ...
    public sealed partial class Scenario2_Client : Page
    {
        ...
        private async void ValueChangedSubscribeToggle_Click()
        {
            if (!subscribedForNotifications)
            {
                //Create BLEoutput file in order to write to it
                Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
                Windows.Storage.StorageFile sampleFile = await storageFolder.CreateFileAsync("BLEoutput.txt", Windows.Storage.CreationCollisionOption.ReplaceExisting);
                //due to window's security settings, the file will be made in C:\Users\...\AppData\Local\Packages\BluetoothLE.PolarStrap_8wekyb3d8bbwe\LocalState
                // look into getting access to put the file in an easier spot
        ...
        private async void Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            // Open txt file in order to enter heart rate data
            Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
            Windows.Storage.StorageFile sampleFile = await storageFolder.GetFileAsync("BLEoutput.txt");

            using (StreamWriter stream = File.AppendText(sampleFile.Path))
            {
                stream.WriteLine($"{DateTime.Now:hh:mm:ss.FFF}\t{newValue}");
            }
        ...
        {            
            byte flags = data[0];
            bool rr = (flags & (1 << 4)) != 0;   // check that there there is an RR value in the flag byte
            bool rz = (flags & (1 << 3)) == 0;   // check that there is not battery or power uint16 value
            

            if (rr && rz)
            {
                int len = data.Length;          // if the data.Length = 5, then there are 2 rr intervals, else it's just 1

                if (len > 4)
                {
                    ushort value1 = BitConverter.ToUInt16(data, 2);     //rr values are 2 hexidecimal units
                    ushort value2 = BitConverter.ToUInt16(data, 4);
                    ushort hr = data[1];

                    object obj1 = (uint)value1;
                    int rr1 = Convert.ToInt32(obj1);

                    object obj2 = (uint)value2;
                    int rr2 = Convert.ToInt32(obj2);

                    double ans1 = rr1 / 1.024;          // convert rr value into milliseconds from # of refreshes
                    double ans2 = rr2 / 1.024;

                    string value = $"{hr}\t{ans1}\t{ans2}";    //string to be displayed and sent to txt file

                    return value;
                }
                else
                {
                    ushort value1 = BitConverter.ToUInt16(data, data.Length - 2);
                    ushort hr = data[1];

                    object obj1 = (uint)value1;
                    int rr1 = Convert.ToInt32(obj1);

                    double ans = rr1 / 1.024;              // convert rr value into milliseconds from # of refreshes

                    string value = $"{hr}\t{ans}";         //string to be displayed and sent to txt file
